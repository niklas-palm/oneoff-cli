    ...
    for job in merged_jobs:
        if isinstance(job["created"], str):
            try:
                ...
                job["created"] = (
                    datetime.min
                )  # Set a default value to avoid breaking sorting
        elif not isinstance(job["created"], datetime):
            click.secho(
                f"Error: Unexpected type for 'created': {type(job['created'])}",
                fg="red",
            )
            job["created"] = (
                datetime.min
...
def create_temp_dockerfile(
    requirements_path: str, script_path: str, script: str
) -> str:
    """
...
def build_and_tag_docker_image(dockerfile_path, image_name, tag):
    ...
    try:
        ...
        client = docker.from_env()
        client.images.build(
            path=os.path.expanduser(os.path.dirname(dockerfile_path)),
            tag=image_name,
...
def run_task(
    region, cluster_name, task_definition_name, subnet_id, security_group_id, name
):
    ...
    try:
        ...
        if failures:
            for failure in failures:
                click.secho(
                    f"Failure: {failure.get('reason')} in {failure.get('arn')}",
                    fg="red",
                )
        ...
        click.secho(
            f"An unexpected error occurred when running the task: {str(e)}", fg="red"
        )
        sys.exit(1)
...
def get_latest_logs(
    log_group_name, log_stream_name=None, start_time=None, end_time=None, limit=10
):
    """
    ...
    try:
        ...
        click.echo(
            f"Log group {log_group_name} or log stream {log_stream_name} not found: {str(e)}"
        )
        return []
    except Exception as e:
...
def kill_job(name: str, region: str, cluster: str, task_arn: str) -> None:
    ...
    try:
        ...
        if error_code == "ResourceNotFoundException":
            ...
            click.secho(
                f"Error: Invalid parameters provided for task ARN '{task_arn}'.",
                fg="red",
            )
        else:
        ...
        click.secho(
            f"Error: An unexpected error occurred while trying to stop task '{task_arn}': {e}",
            fg="red",
        )
